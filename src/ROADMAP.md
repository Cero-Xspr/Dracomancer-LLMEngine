...
#### 能效头对头 ✅ 首测（2026-09-19 傍晚，qwen35moe，空载 AC 同轮）
- **dengine vs llama.cpp CPU 解码 J/token**（draco tune，总口径=含空闲底噪/边际口径=模型自身计算）：
  | 引擎 | tok/s | J/token 总 | J/token 边际 |
  |---|---|---|---|
  | dengine（active 自旋） | 9.7 | 2.572 | 0.616 |
  | dengine（**passive**） | 8.8 | 2.854 | **0.221** |
  | llama.cpp CPU 最优 | 15.6-16.2 | **1.601** | 0.250-0.493 |
- **结论**：①总口径 llama.cpp 赢（他们快，25W 空闲底噪摊得更薄）——**能效第一杠杆还是速度**；
  ②边际口径（模型自身能耗）我们 passive 后 **0.221 J/tok 反超 llama.cpp**——自旋等待白烧的电
  占模型边际能耗的 ~2/3；③`DRACO_WAIT_POLICY=passive` 是新旋钮（autotune 加了环境覆盖口子），
  代价 -9% 速度；-P eco 档未来可接它。
- 注意：J/token 单次测量噪声 ±30%，上述比值（尤其边际 2.8× 改善）机理明确可信，绝对值别跨次比。

### 🔍 投机解码对 MoE：文献调研 + 我们的场景判定（2026-09-19 用户提议）
- **MoESD**（arXiv 2505.19645，清华+华为，NeurIPS'25 Spotlight）：证明 MoE 在**中等 batch**下比稠密模型更受益于 SD——
  因为 batch 大到"所有专家都被激活"时，验证多草稿 token **不增加专家加载开销**，验证计算近乎免费；
  越稀疏（K/E 越小）潜力越大。**结论对我们的适用性：不适用**——我们是 batch=1（本地单用户），
  batch=1 时每 token 只激活 K=8 个专家，验证 t 个草稿 token 会激活最多 t·K 个专家并集 → 加载量线性涨。
  他们的收益来自服务器吞吐场景（vLLM/SGLang 多请求批处理），我们的场景没有那个 batch。
- **SS-MoE**（WWW'26，"Self-Speculative Decoding for On-device MoE Acceleration"）：**这个命中我们的场景**。
  核心机制：①草稿阶段只用 **top-r 专家**（r<k），验证用完整 top-k —— 因 top-r ⊂ top-k，验证只需**增量加载 (k−r) 个专家**；
  ②专家权重全存 CPU 内存、GPU 显存做 **LRU 专家缓存**；③置信度自适应接受（高置信直接收，低的才验证）；
  ④top-k/r 预取（top-1/2 实测 86% GoodPrefetch——草稿激活的专家在验证时大概率被重新激活）。
  实测 RTX 4090 上 Moonlight-16B-A3B 达 3.72×。**对我们的适配点**：我们是纯 CPU，无 GPU↔CPU 搬运问题，
  但"专家并集加载"的账完全一样，且当**模型超出可用内存时**（见下）SS-MoE 的"减少 I/O"直接命中。
- **DraftExpert**（arXiv 2607.24484）：把专家卸载式 MoE 的 SD 形式化为**专家集扩张问题**，用 router 一致性蒸馏
  预测验证阶段的专家集以做预取 + expansion-aware truncation。
- **★ 关键实测（2026-09-19）**：K2-Horizon-MoVA-36B-A4B-Q4_K_M = **22.4GB**，本机 22GB 内存 + 10GB available
  ⇒ **模型装不进内存，每 token 激活的专家（~2GB）要从磁盘读**（实测 decode ~0.4s/token ≈ 5GB/s，与我们测过的
  S770 顺序读 6.4GB/s 吻合）⇒ **K2 的推理瓶颈是磁盘 I/O 而不是算力**。
- **行动项（K2 适配完成后）**：①先试更小的量化（NANI 有 IQ2_M 12.5GB / Q2_K_S 11.1GB）看是否可全驻留——
  若驻留则速度天然提升数倍（最简单的一步）；②若仍需磁盘流式，则 SS-MoE 的 top-r 草稿 + 专家 LRU 缓存 +
  预取是**对症**的方案；③我们有 chunked prefill 的批处理基建，将来若做多用户并发，MoESD 的 medium-batch 收益会重现。

## F. 多硬件协作（2026-09-20 立项；MI50 雷电外接在途）
...
| 算子 | 亲和序 | 依据 |
|---|---|---|
| MoE 专家 gemv（字节大头） | iGPU > MI50 >> CPU > NPU | 纯流式，看带宽 |
| attention（小矩阵+延迟敏感） | CPU ≈ iGPU | 延迟型 |
| 路由/softmax/胶水 | CPU | 逻辑串行 |
| KV 读写 | 与注意力同侧 | 就近 |
| prefill 批量 GEMM | MI50 >> iGPU > CPU | 算力型 |

**里程碑**：
- F1: dengine iGPU Vulkan gemv（IQ2_S 先行，GRIDSGN 表放 shared/LDS；RADV 无 VNNI 走 f32 累加）
- F2: 算子拆分调度——专家→iGPU、注意力+胶水→CPU，单 token 流水（目标 1.8× ≈ 5-6 t/s）
- F3: NPU 角色——prefill 的批处理或能效档（GEMV 不勉强）
- F4: **MI50 到货**——专家迁移 HBM2（1TB/s，IQ2_M 12.4GB 全驻），iGPU+CPU 管注意力/胶水；三硬件聚合上限 = min(总线 128GB/s, HBM2 1TB/s) ⇒ decode 理论 100+ t/s
- 验证：每阶段过 k2_gate（逐层校验和）+ k2_prefill_gate；dengine 作 oracle，与 llama.cpp（若 k2 上游）互为参照
