...
rc = vk.lib.vg_init(ct.byref(hp), BUF, k2_vk.SPV.encode(), k2_vk.SPV3S.encode(),
                        k2_vk.SPVG.encode(), X_FLOATS * 4, Y_FLOATS * 4, 64 << 20, ct.byref(info))
assert rc == 0, f"vg_init {rc}"
vk.h = hp
vk.n_w = info.n_w
print(f"[st] W 缓冲 {info.n_w} 块")
g16 = np.fromfile(k2_vk.G16, np.uint16).astype(np.uint32)
assert vk.lib.vg_upload_grid(vk.h, g16.ctypes.data_as(ct.c_void_p), g16.size * 4) == 0
