...
def t1_k2_greedy():
    """k2（K2-Horizon-MoVA-36B）金标准 = numpy oracle（torch 机制对拍 max|Δ|=1.4e-7）的贪心。"""
    gold = _load_golden("k2horizon_golden")
    if gold is None:
        return False, "缺 tests/golden/k2horizon_golden.json"
    m = "/media/xiao_/OverSys1/gguf/k2-horizon/K2-Horizon-MoVA-36B-A4B-Q4_K_M.gguf"
    if not os.path.exists(m):
        return False, f"缺 K2 模型（{m}）"
    s = _find_script("k2_gate.py")
    rc, out = _run([PY, s], timeout=3600, env={"MODEL": m})
    return rc == 0, out
