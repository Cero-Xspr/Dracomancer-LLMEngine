...
RECR = [bool(int(v)) for v in kv("attention.recurrent_layers")][:NL]
...
print(f"[CFG] qwen35: 主图层={NL}(+MTP {NEXTN}) H={H} 头={NH}/{NKV}×{HD_A} rope={ROT}@{ROPE_BASE:.0e} "
      f"FF={FF} GDN层={sum(RECR)} 全注意力层={[i for i in range(NL) if not RECR[i]]} vocab={VOCAB}", flush=True)
...
def forward(tid, pos):
    ...
    for il, L in enumerate(LAYERS):
        ...
        M6E.m6_dense_op(pf(X2N), L["dp"], pf(FFO))
        X[:] = X2O + FFO
