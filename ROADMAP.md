...
**遗留（精确交接）**：pos0 逐层 cos 呈非单调模式——**层 10~30 全 0.998+（纯 Q6_K 权重），
层 0~9 与 31~39 掉到 0.6~0.97**（这些层混 Q8_0/Q5_K/IQ4_XS/Q4_K）。非单调 ⇒ 不是累积漂移，
是**某个非 Q6_K 量化码的 gemv 路径在本模型形状上有 bug**。下一步：逐 (类型×形状) 内核矩阵——
对本模型实际用到的每个 (量化类型, n_out, n_in) 组合，用反量化 numpy 参考逐一对账
（Q8_0 54 个/Q5_K 54/IQ4_XS 15/Q4_K 9 的张量在哪些层哪些位置 → 定位坏内核 → 修）。
修完再：贪心金标准 → 登记（_DENGINE_ADAPTERS_ARCH 已支持）→ T1/T2 用例。
