...
**下一步选项**（未开工）：CPU 侧做访存/计算重叠（软预取下一步的权重块）把 11→15+ GB/s；
或接受现状（iGPU 后端本来就通）转阶段 4（AVX2 回退）。
...
- **Q8_0 全形状 cos 1.000 ✓**；**K 系（Q4_K/Q5_K/Q6_K）标量版与 gguf-py 不一致**
  ...
  **下一脚（决定性、快）**：把同一合成块喂给 C 标量 gemv + 单热提取，
  与 gguf-py 的映射逐元素比——C 与合成映射一致 ⇒ 矩阵验证脚本有 bug；不一致 ⇒ C 块间 stride 错。
  工具：/tmp/synth_map.py（合成映射提取）、/tmp/scalar_verify.py（矩阵，注意 n_in/n_out 方向已修正）。
